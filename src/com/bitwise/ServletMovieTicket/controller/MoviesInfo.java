...
public class MoviesInfo {
	...
	public boolean bookingData(String customerName, String customerAddress, int seats, String movieShow, String date) {
		...
		if (checkSeatsAvailability(movieShow, seats)) {
	...
	public boolean checkSeatsAvailability(String movieShow, int seats) {
		for (Movies m : list) {
			if ((m.getMoviename().equals(movieShow)) && m.getTotalSeats() >= seats) {
				m.setTotalSeats((m.getTotalSeats() - seats));
				return true;
			}
		}
